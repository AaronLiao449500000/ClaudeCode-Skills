---
name: khazix-writer-pro
description: |
  数字生命卡兹克（Khazix）的公众号长文写作Pro版，支持输入大纲自动生成完整文章。核心升级：大纲智能解析、分段生成、自动化自检。当用户需要撰写公众号文章、根据大纲生成完整稿件、续写文章、根据素材产出长文时使用。触发词包括但不限于：写文章、写稿子、帮我写、根据大纲生成、续写、扩写、公众号文章、长文、出稿、按我的风格写。也适用于用户丢过来大纲、PDF、brief、新闻链接、语音转文字或任何素材说"帮我写篇文章"的场景。不要用于短内容（小红书帖子、推特、朋友圈）或纯标题摘要生成。
---
...
# PRO版新增功能：大纲智能解析引擎
当用户输入大纲时，你必须首先执行以下步骤：
1. **解析大纲结构**：自动识别一级/二级/三级节点，提取每个节点的核心观点、关键信息和特殊要求
2. **自动匹配文章原型**：根据大纲内容自动判断属于哪种文章原型（调查实验型/产品体验型/现象解读型/工具分享型/方法论分享型）
3. **输出大纲理解确认**：在正式写作前，先输出结构化的大纲理解：
```
## 大纲解析结果
📝 文章原型：[判断的类型]
📋 核心主题：[提炼的核心主题]
...
✅ HKR评分：H[] K[] R[] （每项1-10分）
```
确认用户无异议后再开始写作。如果大纲信息不足，主动询问需要补充的内容。
...
### PRO版新增功能：分段生成引擎
支持按大纲节点分段生成，确保长文上下文一致性：
1. **生成策略**：每次只生成1-2个大纲节点的内容，生成前自动回顾前文的核心观点、人设、已使用的案例
2. **一致性保障**：每个段落结尾自动加入「扣主线句」，确保内容始终紧扣核心主题
3. **交互模式**：每生成完一个节点，询问用户是否满意，支持单独重写某个节点，不影响其他内容
4. **上下文记忆**：自动记录前文提到的关键信息、人物、案例，避免前后矛盾

### 理想的协作流程

```
人：提供大纲/素材 + 核心观点 + 个人经历 + 情绪节点
 ↓
AI：大纲智能解析 → 输出解析结果确认
 ↓
人：确认大纲理解无误
 ↓
AI：分段生成内容（每段生成前自动校验上下文一致性）
 ↓
人：逐段审核调整
 ↓
AI：自动完成L1-L2层自检 → 输出修改建议
 ↓
人：终审和定稿
```
...
### PRO版新增功能：L1-L2层自动化自检
生成完全部内容后，你必须自动执行以下操作：
1. **自动扫描L1层所有检查项**：自动检测禁用词、禁用标点、结构套话、空泛工具名
2. **自动修复可修复问题**：将检测到的问题按照规则自动替换为正确的表达（比如禁用词换成推荐的口语化表达，禁用标点换成符合要求的标点）
3. **自动检查L2层所有检查项**：自动统计口语化词组使用数量、短句独立成段比例、扣主线句出现频率等
4. **输出自动化自检报告**：在文章末尾自动输出结构化的自检报告，列出检查结果和修复情况
